...
def pdf_analyse_and_rename(file):
    ...
    if not (("发票" in text) or ("收费票据" in text)):
        return (f"注意：{file}可能不是电子发票原件。如果确认是电子发票原件，请发送报错文件给limengcheng@akic.tech",
                "warning")
    else:
        ...
        if count > 1:
            return (f"注意：{file}可能有多张电子发票或存在备注页，为最大程度避免您的资金损失，建议手动重命名。", "warning")
        ...
        if len(amounts2) == 2 and len(set(amounts2)) == 2:
            return (f"警告：{file}中识别到有效¥字符的金额异常，请手动检查文件以确保所有金额都已正确识别。", "warning")
        ...
        if amounts:
            ...
            if os.path.exists(new_file):
                ...
                try:
                    ...
                    # 捕捉异常并执行相应的操作
                    if e.errno == 63:
                        return (f"发生未知错误{e}，可能是文件名太长无法重命名，请尝试缩短文件名或使用缩写", "warning")
                    else:
                        return (f"发生未知错误{e}，有可能是因为你打开了某个pdf，请关闭后重试", "warning")
        else:
...
if __name__ == '__main__':
    ...
    file="dzfp_24312000000381259558_上海临港益邦智能技术股份有限公司_20241204201232.pdf"
